...
> **Nota:** Para identificar qué ciudades concentran el *mayor volumen* de transacciones (en lugar del mayor monto promedio), se debe reordenar la consulta con `ORDER BY Numero_transacciones DESC` — un análisis pendiente de verificar con datos reales antes de documentarlo aquí.
...
- ¿Qué porcentaje de clientes tiene al menos un préstamo?
- ¿Cuál es el monto promedio de préstamo por rango de `credit_score`?
- ¿Qué clientes tienen tarjeta pero no tienen ningún préstamo?
- ¿Cuántas tarjetas hay por tipo y en qué ciudades están sus clientes?
- ¿Qué cuentas tienen saldo negativo o muy bajo, agrupadas por ciudad del cliente?
